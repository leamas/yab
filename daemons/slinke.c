			...
			*eq = 0;
			...
			if (strcasecmp(buffer, "desc") == 0) {
				...
				*desc = strdup(eq);
				...
				if (!get_data(eq, &r->pre_data, &r->pre_data_bits)) {
					break;
				}
				continue;
				...
				if (!get_data(eq, &r->post_data, &r->post_data_bits)) {
					break;
				}
				continue;
